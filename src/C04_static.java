...
public class C04_static {
	...
	//	  (인스턴스 .을 찍고 사용해도 어차피 다 같은 값을 보기 때문에)
	//	- static 영역의 자원은 인스턴스가 생기기 전에도 호출할 수 있다
	//	- 인스턴스가 존재하지 않을 때도 static이 사용될 가능성이 있기 떄문에
...
//		System.out.println(s3.width);
		...
		//myobj 패키지 내부에 static 변수와 메서드르 가진
		// 클래스를 하나 설계 해보세요
...
class Card {
	...
	public Card(String shape, int number) {
		...
		System.out.println("constructor bloack called");
		
		// 생성자에서 static을 초기화 하는 경우
		// 초기화 되지 않은 static 필드르 사용할 위험이 있다
		// width = 100;
		// height = 200;
	}
	...
	public static void changeCardSize(int width, int height) {
		// static 메서드에서 this가 존재하지 않는다
		// (아직 인스턴스가 생성하지 않았을수도 있으므로)
