...
public class D07_HashMap {
	...
	public static void main(String[] args) {
		...
		//	- entrySet : Enrty<K, V>들로 이루어진 Set을 반환한다
		...
		// # values으로 돌리기
		//	- Key로 Value를 찾는것은 쉽지만 Value로 Key를 찾는것은 불가능하다 (value에는 중복값이 있기때문에)
...
//		map2.put(0, "ABC");
		...
		// # containsKey
		System.out.println(map1.containsKey("사람나이"));
		System.out.println(map1.containsKey("사람나이 "));
		...
		// # get메서드를 사용하다가 데이터가 혹은 키가 없는 경우 null이 반환되어
		// NullPointerException이 발생할 위험이 있다
		
		// # getOrDefault메서드를 이용해 null값이 반환되는 상황이라면 기본값을
		//   반환받을 수 있다
		System.out.println(map1.get("이름").charAt(0));
		System.out.println(map1.getOrDefault("엄아의 이름", "기본값").charAt(1));
		...
		// 키값에는 이름보다 전화번호 같은 값을 사용하는 것이 좋다
