...
public class PokerHandRanking {
	...
	public void check(PokerHand player) {
		...
		if(suitCount.contains(5) && checkMountain(ranks)) {
			...
			// indexof : 앞에서부터 해당 요소를 찾는다	(위치)
			// lastIndexof : 뒤에서부터 해당 요소를 찾는다 	(위치)
...
//		System.out.println(suitCount);
	...
	public static Set<Rank> getMOUNTAIN() {
		return MOUNTAIN;
	}
	public List<Integer> getRankCount() {
		return rankCount;
	}
	public static Set<Set<Rank>> getSTRAIGHTS() {
		return STRAIGHTS;
	}
	public List<Integer> getSuitCount() {
		return suitCount;
	}
}
